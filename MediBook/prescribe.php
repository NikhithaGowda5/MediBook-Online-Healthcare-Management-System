...
if(isset($_POST['prescribe']) && isset($_POST['pid']) && isset($_POST['AppID']) && isset($_POST['appdate']) && isset($_POST['apptime']) && isset($_POST['lname']) && isset($_POST['fname'])){
  ...
  $pid = $_POST['pid'];  // Get patient id from POST data
// Check if the PID exists in the patient table
$check_pid_query = mysqli_query($con, "SELECT * FROM patient WHERE pid = '$pid'");
if (mysqli_num_rows($check_pid_query) == 0) {
    echo "<script>alert('Patient not found.');</script>";
} else {
    $query = mysqli_query($con, "INSERT INTO prescriptiontable (doctor, pid, AppID, fname, lname, appdate, apptime, disease, allergy, prescription)
                                 VALUES ('$doctor', '$pid', '$AppID', '$fname', '$lname', '$appdate', '$apptime', '$disease', '$allergy', '$prescription')");
    if ($query) {
...
<head>
  ...
  <style type="text/css">
    ...
    .nav-link:hover {
      background-color:rgb(77, 233, 63);
    }
